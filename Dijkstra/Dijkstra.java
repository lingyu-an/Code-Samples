...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) { 
	  	// formula for computing Euclidean distance according to Wikipedia
  ...
  public void computeAllEuclideanDistances() {
	  ...
	  // compute distance for every edge of every vertex
	  for (Vertex v : getVertices())
			for (Edge e : v.adjacentEdges)
			{
				Vertex z = e.target;
				e.distance = computeEuclideanDistance(v.x, v.y, z.x, z.y);
			}
  }
  ...
  public void doDijkstra(String s) {	  
	...
	// set all vertices to unknown with distance of infinity
	...
	// starting vertex
	Vertex c = vertexNames.get(s);
	c.distance = 0.0;
	c.known = true;
	pq.offer(new VertexCost(0.0, c));
	    
	// while there is an unknown vertex
	while (!pq.isEmpty()) {
	    VertexCost next = pq.poll();
	    c = next.vertex;
	    c.known = true;
	    	
	   	// for each adjacent vertex
	    for (Edge cv : c.adjacentEdges) {
	    	Vertex v = cv.target;
	    		
	    	// update if vertex is unknown and distance is less than before 
	    	if (!v.known && (c.distance + cv.distance < v.distance)) {
	    		v.distance = cv.distance + c.distance;
	    		v.prev = c;
	    		pq.offer(new VertexCost(v.distance, v));
	    	}
	    }
	}
  }
  
  // subclass with vertex and comparable cost variable for the priority queue
  private class VertexCost implements Comparable<VertexCost> {
	  Vertex vertex;
	  double cost;
	  ...
	  public int compareTo(VertexCost x) {
		  	return Double.compare(cost, x.cost);
	  }
}
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    // list to store edges that get added to the path.
    List<Edge> path = new LinkedList<>();
  	Vertex v = vertexNames.get(t);

  	// trace by prev pointers
  	while (v.prev != null) {
  		Edge e= new Edge(v.prev, v, computeEuclideanDistance(v.prev.x, v.prev.y, v.x, v.y));
  		path.add(e);
  		v = v.prev;
  	}
  	return path;
